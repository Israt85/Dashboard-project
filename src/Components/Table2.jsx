...
import deletesvg from '../assets/deletesvg.png'
...
const Table2 =()=>{
    ...
    const handleDelete = async (id) => {
        ...
        if (isConfirmed) {
            try {
                const res = await axios.delete(`https://api.restful-api.dev/objects/${id}`);
...
<AddProduct users={users}></AddProduct>
</p>

            </div>
<div className="overflow-x-auto">
  <table className="table bg-gray-50 mx-2">
    {/* head */}
    <thead>
      <tr>
        <th>SL</th>
...
<tr key={user.id} className="hover:bg-gray-100">
        <th>{user?.id}</th>
        <td>{user?.name}</td>
        <td>{user?.data?.color || 'N/A'} </td>
        <td>{user?.data?.capacity || 'N/A'} </td>
        <td>{user?.data?.price || 'N/A'}</td>
        <img onClick={()=>handleDelete(user?.id)} className="w-6 h-6" src={deletesvg} alt="" />
